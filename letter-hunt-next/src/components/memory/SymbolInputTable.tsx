...
/**
 * Symbol Input Table Component
 *
 * For Level 6: User must fill in the symbols for numbers 1-0 from memory.
 * Shows numbers in first row, inputs in second row.
 *
 * @module SymbolInputTable
 */
...
interface SymbolInputTableProps {
  numbers: string[];
  userInputs: string[];
  isValidated: boolean;
...
export function SymbolInputTable({
  numbers,
  userInputs,
  isValidated,
  validationResult,
    ...
    <Card className={cn('p-6', className)}>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-text-primary mb-2">
          Nivel 6: Recuerda los Símbolos
        </h2>
        <p className="text-text-secondary">
          Escribe el símbolo que corresponde a cada número. Debes acertar todos para continuar.
        </p>
      </div>
            ...
            <div key={`input-${num}`} className="relative">
              <input
                ...
                className={cn(
                  ...
                  'focus:outline-none focus:ring-2 focus:ring-warning-300',
                  cellStatus === 'empty' && [
                ...
                aria-label={`Símbolo para el número ${num}`}
              />
              ...
              {isValidated && cellStatus === 'incorrect' && (
                <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 text-xl text-error-600 font-semibold whitespace-nowrap">
      ...
      {isValidated && validationResult && (
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            >
              {validationResult.correct === 10
                ? '¡Perfecto! Todos los símbolos son correctos'
                : `Tienes ${validationResult.incorrect} error${validationResult.incorrect === 1 ? '' : 'es'}`}
            </span>
          </div>
          <p className="text-sm text-text-secondary">
            Correctos: {validationResult.correct}/10
          </p>
        </div>
      )}
